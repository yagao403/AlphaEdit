...
def compute_z(
    ...
    # Determine devices: model inputs go to the embedding device,
    # delta lives on the target layer's device for hook compatibility.
    ...
    # rewrite layer, will induce the target token to be predicted at the final
    # layer.  delta must live on the same device as the layer output so the
    # edit_output_fn hook can add it without cross-device errors.
    ...
    delta = torch.zeros((hidden_dim,), requires_grad=True, device=layer_device)
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # lm_w and lm_b may be on a different device (lm_head device)
